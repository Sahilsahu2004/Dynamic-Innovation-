...
export default function Hero() {
  ...
  return (
    <section
      id="top"
      ref={ref}
      className="relative min-h-[100svh] overflow-hidden bg-forest pt-28 pb-20 md:pt-32"
    >
      <div className="pointer-events-none absolute inset-0 opacity-[0.05]">
        <div
          ...
          style={{
            backgroundImage:
              'repeating-linear-gradient(0deg, #F7F7F1 0px, #F7F7F1 1px, transparent 1px, transparent 64px), repeating-linear-gradient(90deg, #F7F7F1 0px, #F7F7F1 1px, transparent 1px, transparent 64px)',
          }}
        />
      </div>
      ...
      <motion.div style={{ opacity: fade }} className="relative mx-auto grid max-w-7xl items-center gap-16 px-6 md:px-10 lg:grid-cols-[1.05fr_0.95fr] lg:gap-10">
        <div className="flex flex-col justify-center">
          <motion.p
            ...
            className="mb-6 font-mono text-[12.5px] tracking-wide text-lime"
          >
            {MANUFACTURER.join('  ·  ')}
          </motion.p>

          <h1 className="font-display font-extrabold text-cream leading-[0.98] text-[12vw] sm:text-[8vw] md:text-[52px] lg:text-[58px] xl:text-[64px]">
            <RevealText text="Premium woven" delay={0.15} />
            <br />
            <span className="text-lime"><RevealText text="packaging" delay={0.35} /></span>
            {' '}
          ...
          >
            <motion.a
              ...
              href="#enquiry"
              className="bg-lime px-7 py-3.5 text-[14px] font-semibold text-forest-deep"
            >
              Get a Quote
            </motion.a>
            <motion.a
              whileHover={{ y: -3, backgroundColor: 'rgba(247,247,241,0.08)' }}
              whileTap={{ scale: 0.97 }}
              transition={{ type: 'spring', stiffness: 400, damping: 20 }}
              href="#products"
          ...
          >
            {['Industrial', 'Agricultural', 'Packaging'].map((tag) => (
