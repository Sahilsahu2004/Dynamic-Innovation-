    ...
    <section className="bg-cream py-24 md:py-28">
      <div className="mx-auto max-w-7xl px-6 md:px-10">
        <motion.p
          ...
          viewport={{ once: true }}
          className="font-mono text-[12.5px] tracking-wide text-lime-deep"
        >
          Indicative Specifications
        </motion.p>
        <h2 className="mt-3 font-head font-bold text-[30px] leading-tight text-forest sm:text-[36px]">
              ...
              {ROWS.map((row, i) => (
                ...
                >
                  {row.map((cell, j) => (
                    <td
                      key={j}
                      className={`p-4 text-[13.5px] ${j === 0 ? 'font-semibold text-forest' : 'text-charcoal/75'}`}
                    >
                      {cell}
                    </td>
                  ))}
